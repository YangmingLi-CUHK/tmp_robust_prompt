...
class GPPTPrompt(torch.nn.Module):
    def __init__(self, n_hidden, center_num, n_classes, device=None, filter_module=None):
        ...
        self.last_filter_detection = None
    ...
    def _maybe_filter_edge_index(self, h, edge_index):
        ...
        if edge_mask is not None:
            self.last_filter_detection = {
                "edge_index": edge_index.detach(),
                "edge_mask": edge_mask.detach(),
                "pred_anomaly_mask": (~edge_mask).detach(),
                "edge_score": filter_output.get("edge_score", None).detach() if isinstance(filter_output, dict) and filter_output.get("edge_score", None) is not None else None,
                "edge_similarity": filter_output.get("edge_similarity", None).detach() if isinstance(filter_output, dict) and filter_output.get("edge_similarity", None) is not None else None,
            }
