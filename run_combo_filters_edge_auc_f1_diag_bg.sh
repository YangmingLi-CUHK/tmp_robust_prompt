...
LOG_DIR="logs/combo_filters_edge_auc_f1_diag"
mkdir -p "$LOG_DIR"
...
run_one() {
  ...
  echo "[$(date '+%F %T')] DONE ${bb} ${tag} ptb=${ptb}"
}
...
for ptb in $PTBS; do
  for bb in stable peak; do
    ...
    run_one "$bb" "$model" all3    0.3  1  0.5 "$ptb"
    run_one "$bb" "$model" sim_deg 0.3  1 -1.0 "$ptb"
    run_one "$bb" "$model" sim_ood 0.3 -1  0.5 "$ptb"
    run_one "$bb" "$model" deg_ood -1.0 1  0.5 "$ptb"
  done
done

grep -h "Edge Detection Detail" "${LOG_DIR}"/*.log \
  > "${LOG_DIR}/edge_detection_detail_lines.txt" || true

python analyze_combo_filters.py \
  --log-dir "$LOG_DIR" \
  --out-prefix "${LOG_DIR}/combo_filters_edge_auc_f1_diag_summary"
