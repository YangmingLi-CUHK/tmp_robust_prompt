...
#
# This is a standalone copy of RobustPrompt_T so the original prompt is untouched.
# The only addition is a new node-level defense tip `nsp_pt` (see add_muti_pt): edges
# whose endpoints have dissimilar higher-order neighbor embeddings are flagged, and
# BOTH endpoints are marked as anomalous nodes that receive a learnable defense prompt
# (same edge -> both-endpoints convention as out_detect_pt). The NSP detection logic is
# ported from Filter2_material/NSPGCN_model.py via prompt_graph/filters/nsp_filter.py.
class RobustPrompt_T_NSP(torch.nn.Module):
    def __init__(self, in_channels: int, muti_defense_pt_dict, p_plus, use_attention, num_heads, cosine_constraint, pt_threshold, temperature, weight_mse, weight_kl, weight_constraint, nsp_order=2, filter_module=None):
        super(RobustPrompt_T_NSP, self).__init__()
        ...
        self.p_plus            = p_plus
        self.nsp_order         = nsp_order           # NSP 邻居嵌入的聚合阶数 (A^order @ X)
        ...
        print('use RobustPrompt Tranductive + NSP')
        ...
        print('nsp_order : ', self.nsp_order)
        ...
        if 'nsp_pt' in self.pt_keys:
            if self.p_plus:
                self.nsp_pt_list = torch.nn.Parameter(torch.Tensor(20, self.in_channels))
    ...
    def add_muti_pt(self, graph, device):
        ...
        # ---------------------------------------------------------------------------
        # NSP 防御 tip：基于高阶邻居嵌入 (A^order @ X) 的边相似度做异常检测。
        # 邻居相似度低于阈值的边被判为异常边 → 边两端节点都标为异常节点（node-level）。
        # 检测逻辑照搬自 Filter2_material/NSPGCN_model.py (见 filters/nsp_filter.py)。
        if 'nsp_pt' in self.pt_keys:
            node_use_nsp_pt, nsp_edge_mask, nsp_edge_sim = nsp_suspicious_nodes(
                x, edge_index, g.num_nodes, self.pt_dict['nsp_pt'], order=self.nsp_order
            )

            if self.p_plus and len(node_use_nsp_pt) > 0:
                score = self.nsp_pt_a(x[node_use_nsp_pt])
                nsp_weight = F.softmax(score, dim=1)
                self.prompt_nsp_pt = nsp_weight.mm(self.nsp_pt_list)

            node_use_pt = torch.concat((node_use_pt, node_use_nsp_pt))
        ...
        self.last_tip_detection = {
            "edge_index": g.edge_index.detach(),
            "node_by_tip": {
                key: value.detach() if isinstance(value, torch.Tensor) else value
                for key, value in node_use_each_pt_whole_graph.items()
            },
            "out_detect_edge_mask": ood_edge_mask.detach() if ood_edge_mask is not None else None,
    ...
    def Tune(self, graph, gnn, answering, lossfn, opi, device):
        ...
        self.last_tau_detection = {
            "edge_index": g_mutiftpt.edge_index.detach(),
            "edge_mask": tune_keep_mask.detach(),
            "pred_anomaly_mask": (~tune_keep_mask).detach(),
            "edge_score": (1.0 - edge_sim_intermediate).detach(),
            "edge_similarity": edge_sim_intermediate.detach(),
        }
