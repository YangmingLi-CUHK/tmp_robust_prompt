#!/usr/bin/env python3
"""Run classic GPromptShield sim/degree tip ablations with fixed SVD100 backbones."""
...
EXPERIMENT_NAME = "citeseer_svd100_to_cora_svd100_transductive_classic_tip_ablation_2bb_2tips_corrected_budget_120"
...
METHODS = (
    {
        "id": "gpromptshield_original_sim_tip_only",
        ...
        "pt_out_detect_threshold": -1,
        "effective_edge_path": (
            "classic_train_tau_tune_0.5;sim_pt_only;other_pt_fallback;"
            "filter_mode_argument_ignored"
        ),
        "prompt_space": "Cora_SVD100_input",
        "description": (
            "Classic implementation with sim_pt as the only active special selector; "
            "degree_pt and out_detect_pt are removed by negative thresholds; "
            "other_pt fallback is retained"
        ),
    },
    {
        "id": "gpromptshield_original_degree_tip_only",
        "prompt_type": "RobustPrompt-T",
        "prompt_variant": "original",
        "filter_mode": "original",
        "active_tip": "degree_pt",
        "pt_sim_threshold": -1,
        "pt_degree_threshold": 1,
        "pt_out_detect_threshold": -1,
        "effective_edge_path": (
            "classic_train_tau_tune_0.5;degree_pt_only;other_pt_fallback;"
            "filter_mode_argument_ignored"
        ),
        "prompt_space": "Cora_SVD100_input",
        "description": (
            "Classic implementation with degree_pt as the only active special selector; "
            "sim_pt and out_detect_pt are removed by negative thresholds; "
            "other_pt fallback is retained"
        ),
    },
)
...
RESULT_FIELDS = [
    ...
    "effective_edge_path", "active_tip", "pt_sim_threshold",
    "pt_degree_threshold", "pt_out_detect_threshold", "epochs",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--plan-only",
        action="store_true",
        help="Print all 120 hard-coded run keys without loading artifacts.",
    )
    parser.add_argument(
        "--preflight-only",
        action="store_true",
...
def build_plan(selected: list[dict[str, object]]) -> list[dict[str, object]]:
    for method in METHODS:
        thresholds = {
            "sim_pt": float(method["pt_sim_threshold"]),
            "degree_pt": float(method["pt_degree_threshold"]),
            "out_detect_pt": float(method["pt_out_detect_threshold"]),
        }
        active_tips = [tip for tip, threshold in thresholds.items() if threshold >= 0]
        if active_tips != [method["active_tip"]]:
            raise RuntimeError(
                f"Each ablation must enable exactly its declared tip: "
                f"method={method['id']}, active={active_tips}."
            )
    plan = []
...
def code_bundle_sha256() -> str:
    paths = [
        ...
        PROJECT_ROOT / "run_citeseer_svd100_to_cora_svd100_transductive_classic_tip_ablation_2bb_2tips_corrected_budget_120.sh",
        ...
        PROJECT_ROOT / "prompt_graph/utils/act.py",
        ...
        PROJECT_ROOT / "data_attack_fewshot/attackdata_specified.py",
...
def config_sha256(context: dict[str, object]) -> str:
    config = {
        ...
        "matrix": "2_fixed_backbones*2_classic_tip_ablations*6_ptb*5_downstream_seeds",
        "methods": METHODS,
        "ptb_rates": PTB_RATES,
        ...
        "robust_prompt_common": {
            ...
            "other_pt": "all_fallback_retained",
        },
        "selector_ablation": {
            method["id"]: {
                "active_tip": method["active_tip"],
                "sim_threshold": method["pt_sim_threshold"],
                "degree_threshold": method["pt_degree_threshold"],
                "out_detect_threshold": method["pt_out_detect_threshold"],
            }
            for method in METHODS
        },
        "disabled_tip_policy": "numeric_threshold_below_zero_removes_tip_from_pt_dict",
...
def write_manifests(
    ...
    manifest_rows = [
        ...
        (
            "matrix",
            "2 fixed backbones * 2 classic selector ablations * "
            "6 corrected-budget ptb levels * 5 downstream seeds = 120 runs",
        ),
        (
            "seed_policy",
        ...
        (
            "target_cache_anchor",
            "target SVD is computed if absent and then fixed for the full local 120-run batch; "
        ...
        ("sim_tip_only", METHODS[0]["description"]),
        ("degree_tip_only", METHODS[1]["description"]),
        (
            "classic_common_settings",
            "prompt_variant=original,prompt_lr=0.01,pt_threshold=0.5,"
            "weight_mse=0.1,weight_kl=0.3,weight_constraint=0.2,temperature=1.0,"
            "p_plus=true,attention=false,cosine_constraint=true",
        ),
        (
            "selector_ablation",
            "sim-only:sim=0.2,degree=-1,out=-1; "
            "degree-only:sim=-1,degree=1,out=-1; negative thresholds remove tips",
        ),
        (
            "ablation_scope",
            "only the sim_pt versus degree_pt special node selector changes; "
            "other_pt=all fallback and classic training-time tau_tune=0.5 remain fixed",
        ),
    ...
    plan_fields = [
        "run_key", "backbone_id", "method", "active_tip", "pt_sim_threshold",
        "pt_degree_threshold", "pt_out_detect_threshold", "ptb_rate", "pretrain_seed",
        ...
        plan_lines.append(
            "\t".join(
                [
                    ...
                    str(run["method"]["id"]),
                    str(run["method"]["active_tip"]),
                    str(run["method"]["pt_sim_threshold"]),
                    str(run["method"]["pt_degree_threshold"]),
                    str(run["method"]["pt_out_detect_threshold"]),
                    str(run["ptb_rate"]),
    ...
    if method["prompt_type"] == "RobustPrompt-T":
        command.extend(
            [
                ...
                "--weight_constraint", "0.2",
                "--temperature", "1.0",
                "--pt_sim_threshold", str(method["pt_sim_threshold"]),
                "--pt_degree_threshold", str(method["pt_degree_threshold"]),
                "--pt_out_detect_threshold", str(method["pt_out_detect_threshold"]),
                "--p_plus",
                "--no_attention",
                "--cosine_constraint",
            ]
...
def base_result_row(
    ...
    return {
        ...
        "active_tip": method["active_tip"],
        "pt_sim_threshold": method["pt_sim_threshold"],
        "pt_degree_threshold": method["pt_degree_threshold"],
        "pt_out_detect_threshold": method["pt_out_detect_threshold"],
        "epochs": 200,
        "shot": 5,
        "split": 1,
        "train_n": 35,
...
def safe_to_skip(
    ...
    guarded_fields = [
        ...
        "filter_mode_argument", "effective_edge_path", "active_tip",
        "pt_sim_threshold", "pt_degree_threshold", "pt_out_detect_threshold",
    ]
    mismatches = [
...
def run_experiment(
    ...
    print(
        ...
        "2 selected checkpoints verified | plan=120"
    )
...
def main() -> int:
    ...
    if args.plan_only:
        print(
            "Formal plan: 2 fixed backbones * 2 classic selector ablations * "
            "6 ptb levels * 5 downstream seeds = 120 runs"
        )
        for run in plan:
            print(run["run_key"])
        return 0
