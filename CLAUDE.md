...
- **NSPGCN（`Filter2_material/NSPGCN_model.py`，实验性/未接入）** — "Neighbor Similarity Preserving GCN"（论文《Towards Robust GSL Beyond Homophily via Preserving Neighbor Similarity》），学习融合多张 kNN 邻接、保留邻居相似度的鲁棒 GSL 方向。**尚未接入 `prompt_graph/`**，为下一步 Filter2 探索的候选。
...
1. **原版复现判定** — 用 hid_dim=64 + `--prompt_variant original` 跑通，最终判定论文 Cora+GraphCL 列的可复现性。
2. **GraphMAE vs GraphCL** — 以 GraphMAE 64-dim backbone 对比 GraphCL，看是否改善 few-shot/鲁棒性。
3. **Filter2 / NSPGCN 接入** — 评估邻居相似度保持（robust GSL）能否作为 filter_module 之外的新防御方向。
4. **面对 GCL 悖论重定义问题** — 高 ptb（≥0.20）下承认基于图的 prompt 不敌 linear probe，聚焦低-中污染或"是否用图"的自适应策略。
...
- **Transductive**（全图，当前主线）：`GPPT`、`RobustPrompt-T`（`--prompt_variant ours|original`）、`RobustPrompt-T-IA`（IA-PT 伪标签扩展）、`RobustPrompt-GPF`、`RobustPrompt-GPFplus`
- **Inductive**（k-hop 子图 batch）：`All-in-one`、`Gprompt`、`GPF`、`GPF-plus`、`RobustPrompt-I`

### RobustPrompt-T 防御机制（对齐 GPromptShield）

**四类防御 prompt**：`sim_pt`（邻居相似度低）、`degree_pt`（低度节点）、`out_detect_pt`（OOD 边端点）、`other_pt`（其余节点增强，p_plus 模式）。
...
### 实验性 / 未接入
- `Filter2_material/NSPGCN_model.py` — 邻居相似度保持 GCN（robust GSL），候选 Filter2 方向，**尚未接入 `prompt_graph/`**
