...
class NodeTask(BaseTask):
      ...
      def _tip_active(self, tip):
            prompt = getattr(self, "prompt", None)
            pt_keys = set(getattr(prompt, "pt_keys", []))
            if tip not in pt_keys:
                  return 0

            pt_dict = getattr(prompt, "pt_dict", {})
            value = pt_dict.get(tip, None)
            if value is None:
                  return 0
            ...
            if tip in ["sim_pt", "out_detect_pt"]:
                  return int(value > -1.0)
            return 0


      def _source_active(self, source):
            if source == "gppt_filter":
                  return int(self.prompt_type == "GPPT")
            if source == "tau_tune":
                  return int(self.prompt_type == "RobustPrompt-T")
            if source in ["sim_pt", "degree_pt", "out_detect_pt"]:
                  return self._tip_active(source)
            return 0


      def _print_edge_detection_metrics(self, source, metrics, active=None, extra=None):
            if metrics is None:
                  return
            if active is None:
                  active = self._source_active(source)
            parts = [
                  "Edge Detection Detail",
                  f"source={source}",
                  f"active={int(active)}",
                  f"n_edges={metrics.get('n_edges', metrics.get('num_edges', 0))}",
                  f"n_pos={metrics.get('n_pos', metrics.get('num_positive', 0))}",
                  f"n_neg={metrics.get('n_neg', 0)}",
                  f"pos_rate={fmt_metric(metrics.get('pos_rate'))}",
                  f"pred_pos={metrics.get('pred_pos', metrics.get('num_predicted', 0))}",
                  f"pred_pos_rate={fmt_metric(metrics.get('pred_pos_rate'))}",
                  ...
                  f"best_precision={fmt_metric(metrics.get('best_precision'))}",
                  f"best_recall={fmt_metric(metrics.get('best_recall'))}",
                  f"best_pred_pos_rate={fmt_metric(metrics.get('best_pred_pos_rate'))}",
                  f"precision_at_k_pos={fmt_metric(metrics.get('precision_at_k_pos'))}",
                  f"recall_at_k_pos={fmt_metric(metrics.get('recall_at_k_pos'))}",
                  f"all_positive_f1={fmt_metric(metrics.get('all_positive_f1'))}",
                  f"current_f1_minus_all_positive_f1={fmt_metric(metrics.get('current_f1_minus_all_positive_f1'))}",
                  f"hard_mask_all_positive_like={metrics.get('hard_mask_all_positive_like', 0)}",
            ]
            if extra:
      ...
      def _print_tip_detection_metrics(self, tip, node_metrics, edge_metrics, active=None):
            if active is None:
                  active = self._tip_active(tip)
            parts = [
                  "Tip Detection",
                  f"tip={tip}",
                  f"active={int(active)}",
      ...
      def _report_edge_detection_metrics(self):
            ...
            if prompt_filter is not None:
                  ...
                  self._print_edge_detection_metrics(
                        ...
                        filter_metrics,
                        active=0,
                        extra={"mode": getattr(prompt_filter, "mode", self.filter_mode)},
                  )
            ...
            if tip_state is not None:
                  ...
                  for tip in ["sim_pt", "degree_pt", "out_detect_pt"]:
                        ...
                        self._print_tip_detection_metrics(tip, node_metrics, edge_metrics, active=active)
                        if tip in ["sim_pt", "degree_pt"]:
                              self._print_edge_detection_metrics(tip, edge_metrics, active=active)
                  ...
                  if ood_mask is not None:
                        ...
                        self._print_edge_detection_metrics(
                              "out_detect_pt",
                              ood_metrics,
                              active=self._tip_active("out_detect_pt"),
                        )
